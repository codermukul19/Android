...
class BoardActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Restarting the game
        FaceResartButton.setOnClickListener{
            gameRestart()
        }
    }
    ...
    private fun setUpBoard(row: Int, col: Int, mine: Int) {
        ...
        mineCountNumber.text = ""+mine

        // Array of buttons to find the position of a particular button
        val cellBoard = Array(row) { Array(col) {MineCell(this)}}

        markFlagButton.setOnClickListener{
            if(choice==1) {
                markFlagButton.setImageResource(R.drawable.flag)
                choice=2
            }else{
                markFlagButton.setImageResource(R.drawable.bomb)
                choice=1
            }
        }
        ...
        for(i in 0 until row){
            ...
            for(j in 0 until col){
                ...
                params2.weight = 1.0F
                button.setBackgroundResource(R.drawable.myten)
    ...
    private fun startTimer(){
        chronometer = findViewById(R.id.timerOut)
    ...
    private fun move(choice: Int, x: Int, y:Int, cellBoard:Array<Array<MineCell>>, rowSize: Int,colSize: Int,mine:Int): Boolean{
        ...
        if(choice == 2){
            ...
            if(cellBoard[x][y].isRevealed) return false
                ...
                flaggedMines--
                cellBoard[x][y].setBackgroundResource(R.drawable.myten)
            ...
            mineCountNumber.text = ""+finalMineCount
            return true;
        }
    ...
    private fun gameWon(isHighScore:Boolean){
        ...
        else builder.setMessage("$lastGameTime is your solving time.")

        builder.setTitle("CONGO!! YOU WON...")
        builder.setCancelable(false)
        ...
        cellBoard.forEach { row ->
            row.forEach {
                if(it.isRevealed)
                    ...
                    FaceResartButton.setImageResource(R.drawable.sad_face)
                ...
                if(status == Status.LOST && it.isMarked && !it.isMine){
                    ...
                    it.setBackgroundResource(R.drawable.flag1)
                    FaceResartButton.setImageResource(R.drawable.won)
                }
                else
                    it.text = " "
            }
    ...
    private fun setNumberImage(button:MineCell) {
        if(button.value==0) button.setBackgroundResource(R.drawable.myzero)
        if(button.value==1) button.setBackgroundResource(R.drawable.myone)
        if(button.value==2) button.setBackgroundResource(R.drawable.mytwo)
        if(button.value==3) button.setBackgroundResource(R.drawable.mythree)
        if(button.value==4) button.setBackgroundResource(R.drawable.myfour)
        if(button.value==5) button.setBackgroundResource(R.drawable.myfive)
        if(button.value==6) button.setBackgroundResource(R.drawable.mysix)
        if(button.value==7) button.setBackgroundResource(R.drawable.myseven)
        if(button.value==8) button.setBackgroundResource(R.drawable.myeight)
